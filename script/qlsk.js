...
async function loadEvents() {
    try {
        ...
        if (!res.ok) {
            alert('Backend lỗi hoặc chưa chạy. Kiểm tra Render.com');
            return;
        }
        ...
        console.error(err);
        alert('Không kết nối được backend để load sự kiện!');
    }
}
...
async function updateEvent(id) {
    ...
    if (document.getElementById('editEventImage').files[0]) {
        formData.append('image', document.getElementById('editEventImage').files[0]);
    }
...
async function openEditModal(id) {
    try {
        ...
        alert('Không load được dữ liệu sự kiện!');
    }
}
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    document.getElementById('eventImage').addEventListener('change', () => {
        const name = document.getElementById('eventImage').files[0]?.name || 'Chưa có ảnh nào được chọn';
    ...
    document.getElementById('editEventImage').addEventListener('change', () => {
        const name = document.getElementById('editEventImage').files[0]?.name || 'Chưa có ảnh nào được chọn';
    ...
    document.body.addEventListener('click', (e) => {
        const target = e.target.closest('.edit-event-btn');
        if (target) openEditModal(target.dataset.id);

        const del = e.target.closest('.delete-btn');
        if (del) deleteEvent(del.dataset.id);

        const see = e.target.closest('.see-btn');
        if (see) openViewModal(see.dataset.id);
    });

    // Search
    document.getElementById('searchInput').addEventListener('input', (e) => {
        const term = e.target.value.toLowerCase().trim();
        // Bạn có thể paste lại hàm searchEvents cũ ở đây nếu muốn giữ hiệu ứng đẹp
    });
...
function updateTabBadges() {
    ...
    document.querySelector('.tab-btn[data-tab="created"] .badge')?.parentElement && 
        (document.querySelector('.tab-btn[data-tab="created"] .badge').textContent = `(${counts.created})`);
    document.querySelector('.tab-btn[data-tab="waitapproved"] .badge')?.parentElement && 
        (document.querySelector('.tab-btn[data-tab="waitapproved"] .badge').textContent = `(${counts.waitapproved})`);
    document.querySelector('.tab-btn[data-tab="approved"] .badge')?.parentElement && 
        (document.querySelector('.tab-btn[data-tab="approved"] .badge').textContent = `(${counts.approved})`);
}
